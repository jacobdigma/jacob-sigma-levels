...
const end = this.page * this.pageSize;return this.filteredLeaderboard.slice(start, end);},entry() {if (this.filteredLeaderboard.length === 0) return null;if (!this.selectedPlayerName) {return this.filteredLeaderboard[0];}const found = this.filteredLeaderboard.find(p => p.name === this.selectedPlayerName);return found || this.filteredLeaderboard[0];},combinedDemons() {if (!this.entry) return [];const list = [];if (this.entry.completed) {this.entry.completed.forEach(s => {list.push({level: s.level,score: s.score,link: s.link,rank: s.rank,isVerified: false,listRank: this.getRankLabel(s.rank)});});}if (this.entry.verified) {this.entry.verified.forEach(s => {list.push({level: s.level,score: s.score,link: s.link,rank: s.rank,isVerified: true,listRank: this.getRankLabel(s.rank)});});}return list.sort((a, b) => b.score - a.score);},stats() {const counts = { main: 0, extended: 0, legacy: 0 };this.combinedDemons.forEach(d => {if (d.listRank.startsWith('#')) counts.main++;else if (d.listRank === 'Extended') counts.extended++;else if (d.listRank === 'Legacy') counts.legacy++;});return counts;}},watch: {search() {this.page = 1;}},methods: {localize,hasCompletedPack(entry, pack) {return checkPackCompletion(entry, pack);},hasAnyPack(entry) {return this.packsConfig.some(pack => this.hasCompletedPack(entry, pack));},selectPlayer(player) {this.selectedPlayerName = player.name;},prevPage() {if (this.page > 1) this.page--;},nextPage() {if (this.page * this.pageSize < this.filteredLeaderboard.length) this.page++;},getRankLabel(rank) {if (!rank) return 'Legacy';if (rank <= this.mainListLimit) {return '#' + rank;} else if (rank <= this.extendedListLimit) {return 'Extended';} else {return 'Legacy';}}},async mounted() {const [leaderboard, err] = await fetchLeaderboard();if (leaderboard) {leaderboard.forEach(player => {let bonusPoints = 0;packsConfig.forEach(pack => {if (checkPackCompletion(player, pack)) {bonusPoints += pack.points;}});player.total += bonusPoints;});leaderboard.sort((a, b) => b.total - a.total);if (leaderboard.length > 0) {this.selectedPlayerName = leaderboard[0].name; // OPRAVENO: Správné nastavení prvního hráče}}this.leaderboard = leaderboard;this.err = err;this.loading = false;},};
