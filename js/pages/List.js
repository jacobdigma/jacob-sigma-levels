            ...
            <div style="flex: 1; background: #ffffff; border: 1px solid #e1e4e8; border-radius: 8px; padding: 25px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); text-align: left; color: #000000; box-sizing: border-box;">
                <div v-if="entry">
                    ...
                    <div v-if="entry.verification && entry.verification !== '#'" style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; background: #000; border-radius: 8px; margin-bottom: 25px;">
                        <iframe style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border: 0;" :src="entry.verification && entry.verification.includes('embed') ? entry.verification : 'https://youtube.com' + (entry.verification && entry.verification.includes('v=') ? entry.verification.split('v=')[1].split('&')[0] : (entry.verification && entry.verification.includes('youtu.be/') ? entry.verification.split('youtu.be/')[1].split('?')[0] : entry.verification))" allowfullscreen></iframe>
                    </div>
    ...
    data() {
        return {
            ...
            list: [
                ...
                { rank: 16, name: "Clutterfunk V2", author: "Neptune", verifier: "Earl12", points: 130, type: "extended", verification: "https://www.youtube.com/watch?v=Kkrp9G-vYeE", records: [ { user: "trumandigma", percent: 100, link: "https://www.youtube.com/watch?v=2kE93FgLRcs" } ] },
                { rank: 17, name: "Electroman Adventures V2", author: "Neptune", verifier: "trumandigma", points: 125, type: "extended", verification: "https://www.youtube.com/watch?v=-LJ3Q-pWZdo", records: [] },
                { rank: 18, name: "Electrodynamix", author: "RobTop", verifier: "trumandigma", points: 120, type: "extended", verification: "https://www.youtube.com/watch?v=fGjDZgluC60", records: [] },
